...
<div id"title" class="title">JEANS ENFANT</div>
...
<div class="contener_slideshow">
    ...
    <center><h2>JEANS ENFANT</h2></center>
    <?php
    ...
    <div class="row">
        <?php $req = $bdd->query('SELECT * FROM produit WHERE id_categorie= 3 AND id_sous_ctg= 4'); ?> <!-- on demande les articles de la table produits -->
        <?php while($row=$req->fetch()) { ?> <!-- fetch permet de recuperer une ligne dans ma base de donnée associé de la boucle while -->
            
            <div class="col-md-3">
                <div id="produits">
                    <div>
                        <img src="photos/enfant/jeans/<?php echo $row['photos_pdt']; ?>" alt=""> 
                    </div>
                    <br />                    
                    <div class="row">
                    ...
                    <div>
                        <a href="jeans_enfant.php?ajouter=<?php echo $row['id_produit'] ?>"> Ajouter au panier </a>
                    </div>
                </div>
            </div>
        <?php }?>
