...
<div class="col-md-12">
<div class="title2">ROBES FEMME</div>
</div>



    <center><h2>ROBES FEMME</h2></center>
    <?php
    ...
    <div class="row">
        <?php $req = $bdd->query('SELECT * FROM produit WHERE id_categorie= 1 AND id_sous_ctg= 6'); ?> <!-- on demande les articles de la table produits -->
        <?php while($row=$req->fetch()) { ?> <!-- fetch permet de recuperer une ligne dans ma base de donnée associé de la boucle while -->
            
            <div class="col-md-3">
                <div id="produits">
                    <div>
                        <img src="photos/femme/robes/<?php echo $row['photos_pdt']; ?>" alt=""> 
                    </div>
                    <br />                    
                    <div class="row">
                    ...
                    <div>
                    <a href="robes_femme.php?ajouter=<?php echo $row['id_produit'] ?>"> Ajouter au panier </a>                    </div>
                </div>
            </div>
        <?php }?>
        </div>
